...
void toPostfix()
{
    ...
    char  postfixExp[MAX_EXPRESSION_SIZE] = '\0'; 
	while(*exp != '\0'){
...
while (postfixStack[top] != lparen) 
printToken(evalPop(&top), &exp); 
evalPop(&top); /* 왼쪽 괄호를 버린다 */ 
}
		}	
while ((token=evalPop(&top)) != '\0') 
printToken(token, &exp); 
printf("\n"); 



printf("postfix expression: %s \n", infixExp);
strcpy(postfixExp, infixExp);
}
